function plot_GaitresultsBinned_2speed_vAUD_overlay(dataIN, GFX_FourierNull, cfg)
...
gaitLines= {':','-'};        % to differentiate single step vs stride.
...
% -------------------------------------------------------------------------
nCols = 3;   % [speed1 | speed2 |Fourier R²]

% Open a figure window
figure(1); clf;
set(gcf, 'Color', 'w', 'Units', 'normalized', 'Position', [.5 0 .5 .9]);
...
if strcmp(cfg.plotlevel, 'GFX')
    ...
    % -----------------------------------------------------------------
    %%  GFX layout (single-line per speed (2x3 grid), step-stride-cps (overlay)
    % -----------------------------------------------------------------
    ...
    for iSpeed = cfg.ispeed
        for nGaits_toPlot = cfg.nGaitstoplot
            ...
            % ---------------------------------------------------------
            %%  GFX single-line: combined foot, one subplot per speedx gait combo
            % ---------------------------------------------------------
            ...
            % Data panel
            ax = subplot(2, nCols, nGaits_toPlot+ nCols*(iSpeed-1));
            hold(ax, 'on');
            ...
            if cfg.plotFIT
                ...
                h(2).LineStyle = gaitLines{nGaits_toPlot};
            ...
            % -------------------------------------------------
            if cfg.plotFITforced && cfg.usebin && ~isempty(GFX_FourierNull) %&& nGaits_toPlot==2 % stride only.

                Hzspace = 0.01:0.2:10;   % must match j6 frequency grid
                axF = subplot(2, nCols, nCols+ nCols*(iSpeed-1));
                hold(axF, 'on');
                ...
                xlabel(axF, 'Frequency (step/stride cycles per stride)');
                ylabel(axF, 'R^2');
                ylim(axF, [0 1]);
                ...
                if nGaits_toPlot==2 
                    ...
                    legend({'Step (fit)','Step (95% CV)','Stride (fit)','Stride (95% CV)'})

                end
                % %% also add the target counts for this combo?
                % usefieldc = '_binned_countsCalc'; 
                %   fldObs  = [cfg.type 'Ons_' usegaitnames{nGaits_toPlot} ...
                %     usefieldc '_fitsRsq_Obs'];
                % fitsObs = GFX_FourierNull(iSpeed).(fldObs);
                % 
                % plot(axF, Hzspace(1:length(fitsObs)), fitsObs, ...
                %     'Color', 'k', 'LineWidth', 3);
